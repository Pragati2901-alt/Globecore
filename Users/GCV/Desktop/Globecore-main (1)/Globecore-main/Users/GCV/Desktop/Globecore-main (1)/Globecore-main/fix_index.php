...
// Replace the buggy headers in index.html
$startMarker = "<!-- ===================== 1. TOP UTILITY BAR ===================== -->";
$endMarker = "</header>";

$startPos = strpos($indexHtml, $startMarker);
$endPos = strpos($indexHtml, $endMarker, $startPos);

if ($startPos !== false && $endPos !== false) {
    // Add len of </header> which is 9
    $newHtml = substr_replace($indexHtml, $navHtml, $startPos, ($endPos + 9) - $startPos);
    file_put_contents($baseDir . "/index.html", $newHtml);
    echo "Successfully updated index.html with relative paths!\n";
} else {
    echo "Could not find header markers.\n";
}
?>
