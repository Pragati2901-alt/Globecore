<?php
// Advanced Generator script for all GlobeCoRe pages with Authentic Website Content & SEO

$baseDir = __DIR__;

// Scraped Core Text from globecoreinc.com
$scraped_mission = "Where there is a thirst, there is a need. We support diverse individuals, couples, families, groups, and organizations locally and abroad to develop and implement sustainable solutions to the challenges of life and new life directions. With our psychological services in Atlanta, GA, we are committed to diversity, cultural inclusion and humility. Our work extends across the lifespan to children, adolescents, adults, and aging populations.";

$scraped_services_text = [
    "counseling" => "Our counseling services are tailored to you. $scraped_mission We empower you to overcome obstacles and find a new life direction.",
    "psychological-evaluations" => "We offer brief psychological evaluations and verification of diagnosis to individuals dealing with cognitive, academic, behavioral, social-emotional, substance abuse, domestic violence, trauma, psycho-sexual, bariatric, veteran, or parental fitness deficits. We service children, adolescents, and adults from diverse cultural backgrounds.",
    "medication-management" => "We offer Medication Management via Naturopathic and Psychiatric modalities. This includes assessment, review, planning, intervention and monitoring to assist in achieving healthier outcomes. Our medication management services pair well with our Therapy & Psychological Evaluation services.",
    "tms-therapy" => "We offer advanced TMS therapy that's gentle, FDA-approved, and tailored to you. By targeting key brain areas, we help lift your mood, overcome treatment resistance, and restore balance, with expert care and genuine compassion.",
    "personal-development" => "We offer customized Personal Development workshops and retreats (in half/full/multi-day formats), on any psycho-social topic designed to meet the needs of individuals looking for sustainable solutions.",
    "professional-development" => "We offer Corporate Development workshops, training, and seminars on psycho-social topics, diversity, equity, and inclusion, meticulously designed to meet the needs of organizations locally and abroad."
];

// Configuration
$services = [
    ["title" => "Counseling", "slug" => "counseling", "keyword" => "Counseling Services Atlanta GA"],
    ["title" => "Psychological Evaluations", "slug" => "psychological-evaluations", "keyword" => "Psychological Evaluation Atlanta"],
    ["title" => "Medication Management", "slug" => "medication-management", "keyword" => "Psychiatric Medication Management Atlanta"],
    ["title" => "TMS Therapy", "slug" => "tms-therapy", "keyword" => "TMS Therapy Atlanta GA"],
    ["title" => "Personal Development", "slug" => "personal-development", "keyword" => "Personal Development Programs Atlanta"],
    ["title" => "Professional Development", "slug" => "professional-development", "keyword" => "Corporate Training Atlanta"]
];
...
if (strpos($headContent, '<!-- SCHEMA_PLACEHOLDER -->') === false) {
...
function buildPage($head, $nav, $footer, $metaTitle, $title, $subtitle, $contentHTML, $faqs, $heroImg, $depth = 1) {
    // Inject Meta Title
    ...
    // Standardize assets just in case
    $head = str_replace('"assets/', '"/assets/', $head);
    $nav = str_replace('"assets/', '"/assets/', $nav);
    $footer = str_replace('"assets/', '"/assets/', $footer);
    
    // Map exact root links
    $head = str_replace('href="/"', 'href="' . $prefix . 'index.html"', $head);
    $nav = str_replace('href="/"', 'href="' . $prefix . 'index.html"', $nav);
    $footer = str_replace('href="/"', 'href="' . $prefix . 'index.html"', $footer);
    $contentHTML = str_replace('href="/"', 'href="' . $prefix . 'index.html"', $contentHTML);
    
    // Map all absolute paths to relative paths based on depth
    $head = str_replace('href="/', 'href="' . $prefix, $head);
    $head = str_replace('src="/', 'src="' . $prefix, $head);
    $nav = str_replace('href="/', 'href="' . $prefix, $nav);
    $nav = str_replace('src="/', 'src="' . $prefix, $nav);
    $footer = str_replace('href="/', 'href="' . $prefix, $footer);
    $footer = str_replace('src="/', 'src="' . $prefix, $footer);
    $contentHTML = str_replace('href="/', 'href="' . $prefix, $contentHTML);
    $contentHTML = str_replace('src="/', 'src="' . $prefix, $contentHTML);
    
    if (strpos($heroImg, '/') === 0) {
        $heroImg = $prefix . ltrim($heroImg, '/');
    }
...
// ========================
// 1. GENERATE SERVICES (Using scraped content)
// ========================
ensureDir($baseDir . "/services");
foreach($services as $svc) {
    $topic = $svc['title'];
    $slug = $svc['slug'];
    $actualContent = $scraped_services_text[$slug] ?? $scraped_mission;
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center">
        <div class="lg:col-span-7 space-y-8">
            ...
            <div class="prose prose-lg text-gray-600">
                <p class="text-xl text-[#071e26] font-semibold leading-relaxed border-l-4 border-primary pl-4 mb-6">$actualContent</p>
                ...
                <p>When you seek $topic, it is crucial to find a team that prioritizes both scientific rigor and empathetic understanding. We provide targeted interventions ranging from advanced therapies to cutting-edge holistic support mechanisms, specifically formulated to promote sustainable wellness.</p>
            </div>
...
HTML;
    ...
    $faqs = [
        ...
        ["q" => "Is telehealth available for $topic?", "a" => "Yes. We offer secure, HIPAA-compliant telehealth options for $topic locally and across multiple states."],
    ...
    $html = buildPage($headContent, $navHtml, $footerHtml, $topic, "$topic Services", "Clinical Expertise", $content, $faqs, getRandomImage(), 1);
    file_put_contents($baseDir . "/services/" . $slug . ".html", $html);
}

// ========================
// 2. GENERATE SPECIALTIES (Using scraped mission)
// ========================
...
foreach($specialties as $sp) {
    ...
    <div class="text-center max-w-4xl mx-auto mb-16">
        ...
        <p class="text-xl text-primary font-semibold italic mb-4">"$scraped_mission"</p>
        <p class="text-xl text-gray-600 leading-relaxed font-medium">Navigating <strong>$topic</strong> can be incredibly challenging, but comprehensive and deeply empathetic support is within reach at GlobeCoRe Inc. We are committed to cultural inclusion and humility while addressing your specific needs.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
        <div class="bg-primary/5 p-8 lg:p-12 rounded-3xl border border-primary/10 hover:shadow-xl transition-shadow">
            ...
            <p class="text-gray-600 leading-relaxed mb-4">Treating $topic requires more than standardized protocols; it requires an adaptive, clinically sound perspective. We merge cutting-edge psychiatric knowledge with profound cultural humility to address the distinct intricacies of $topic.</p>
...
HTML;
    ...
    $faqs = [
        ["q" => "Is $topic therapy highly effective?", "a" => "Yes. Clinical efficacy for $topic treatment is exceedingly high when utilizing modern, evidence-based practices and targeted psychiatric evaluations."],
    ...
    $html = buildPage($headContent, $navHtml, $footerHtml, "$topic Treatment", "$topic Therapy", "Targeted Specialized Care", $content, $faqs, getRandomImage(), 1);
    file_put_contents($baseDir . "/specialties/" . $slug . ".html", $html);
}

// ========================
// 3. GENERATE LOCATIONS (Using scraped mission)
// ========================
    ...
    <div class="flex flex-col-reverse lg:flex-row gap-16 items-center">
        ...
        <div class="lg:w-1/2 space-y-6">
            ...
            <div class="bg-gray-50 border-l-4 border-primary p-6 rounded-r-xl my-4">
                <p class="text-primary italic font-medium leading-relaxed">"$scraped_mission"</p>
            </div>
            ...
            <p class="text-lg text-gray-600 leading-relaxed">Whether you are confronting acute anxiety, navigating complex relational dynamics, or seeking FDA-approved TMS Therapy, our clinicians provide encompassing support—accessible securely via our Telehealth portal and at our Lenox Road headquarters.</p>
...
HTML;
    ...
    $html = buildPage($headContent, $navHtml, $footerHtml, "Therapist in $loc, GA", "Therapy in $loc, GA", "Local Mental Health Care", $content, $faqs, getRandomImage(), 1);
...
foreach($otherPages as $page) {
    ...
    <div class="bg-gray-50 rounded-3xl p-10 lg:p-16 border border-gray-100 shadow-sm text-center">
        ...
        <p class="text-xl text-primary font-semibold italic mb-6">"$scraped_mission"</p>
...
HTML;
    ...
    $html = buildPage($headContent, $navHtml, $footerHtml, $topic, $topic, "Help & Information", $content, $faqs, getRandomImage(), $depth);
    file_put_contents($baseDir . "/" . $slug . ".html", $html);
}

echo "All pages beautifully generated with scraped content and SEO optimization!";
?>
