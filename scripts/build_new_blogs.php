<?php
$base_dir = "c:/Users/GCV/Desktop/Globecore-main (1)/Globecore-main";
$assets_dir = $base_dir . "/assets";
$template_file = $base_dir . "/does-tms-therapy-help-anxiety.html";
$blog_index = $base_dir . "/blog.html";
$artifact_dir = "C:/Users/GCV/.gemini/antigravity/brain/cd1a6877-7dc8-40b0-aa2f-3daf493e4cff";

// Copy images to assets
$images = ["psych_eval_blog_1773723989285.png", "psych_abuse_blog_1773724005530.png", "tricare_mental_health_blog_1773724024240.png", "hormonal_mood_blog_1773724039183.png"];
foreach ($images as $img) {
    $src = $artifact_dir . "/" . $img;
    $dst = $assets_dir . "/" . $img;
    if (file_exists($src)) {
        copy($src, $dst);
...
foreach ($blogs as $blog) {
    ...
    $html_out = preg_replace("/<title>.*?<\/title>/s", "<title>{$blog['meta_title']}</title>", $html_out);
    
    // Meta descriptions and tags
    $html_out = preg_replace('/<meta content=".*?" name="description"\/>/', '<meta content="' . $blog["meta_desc"] . '" name="description"/>', $html_out);
    $html_out = preg_replace('/<meta content=".*?" name="keywords"\/>/', '<meta content="' . $blog["keywords"] . '" name="keywords"/>', $html_out);
    $html_out = preg_replace('/<meta content=".*?" property="og:title"\/>/', '<meta content="' . $blog["meta_title"] . '" property="og:title"/>', $html_out);
    $html_out = preg_replace('/<meta content=".*?" property="og:description"\/>/', '<meta content="' . $blog["meta_desc"] . '" property="og:description"/>', $html_out);
    $html_out = preg_replace('/<meta content=".*?" property="og:url"\/>/', '<meta content="https://globecoreinc.com/' . $blog["id"] . '.html" property="og:url"/>', $html_out);
    $html_out = preg_replace('/<meta content=".*?" name="twitter:title"\/>/', '<meta content="' . $blog["meta_title"] . '" name="twitter:title"/>', $html_out);
    $html_out = preg_replace('/<meta content=".*?" name="twitter:description"\/>/', '<meta content="' . $blog["meta_desc"] . '" name="twitter:description"/>', $html_out);

    $dateParts = explode("/", $blog["date"]);
    $dateFormatted = "{$dateParts[2]}-{$dateParts[1]}-{$dateParts[0]}";
    $imagePathClean = str_replace("./", "", $blog["image"]);
...
<script type="application/ld+json">
{
  ...
  "headline": "{$blog['title']}",
  "image": "https://globecoreinc.com/{$imagePathClean}",
  "author": {
    "@type": "Person",
...
EOT;
    ...
    $html_out = str_replace('</head>', $schema_code, $html_out);
    
    // Header category
    $html_out = preg_replace('/<span class="bg-primary\/10 text-primary text-\[10px].*?<\/span>/s', '<span class="bg-primary/10 text-primary text-[10px] font-extrabold px-3 py-1.5 rounded-full shadow-sm uppercase tracking-[0.15em] mb-6 inline-block">' . $blog["category"] . '</span>', $html_out);
    
    // H1
    $html_out = preg_replace('/<h1 class="text-\[32px\].*?<\/h1>/s', '<h1 class="text-[32px] md:text-[42px] lg:text-[48px] font-extrabold text-[#071e26] leading-tight font-raleway mb-8">' . $blog["title"] . '</h1>', $html_out);
    
    // Date
    $html_out = preg_replace('/\d{2}\/\d{2}\/\d{4}<\/span>\s*<span class="w-1\.5/s', $blog["date"] . "</span>\n<span class=\"w-1.5", $html_out);
    
    // Hero Image
    $html_out = preg_replace('/<img alt=".*?" class="w-full h-auto object-cover max-h-\[500px\]" src=".*?"\/>/', '<img alt="' . htmlspecialchars($blog["title"]) . '" class="w-full h-auto object-cover max-h-[500px]" src="' . $blog["image"] . '"/>', $html_out);
    
    // Content string matching logic
    $start_tag = '<div class="article-content text-[17px] md:text-[18px]">';
    $start_idx = strpos($html_out, $start_tag);
    $end_idx = strpos($html_out, '</article>', $start_idx);
    
    if ($start_idx !== false && $end_idx !== false) {
        $new_content = $start_tag . "\n" . $blog["content"] . "\n</div>\n";
        $html_out = substr($html_out, 0, $start_idx) . $new_content . substr($html_out, $end_idx);
    }
    
    $output_path = $base_dir . "/" . $blog["id"] . ".html";
    file_put_contents($output_path, $html_out);
...
$new_index_html = str_replace($injection_point, $injection_point . "\n" . $cards_html, $index_html);
file_put_contents($blog_index, $new_index_html);
echo "Updated blog.html successfully.\n";
?>
